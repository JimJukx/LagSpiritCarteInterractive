  ...
  departmentLayers.forEach((dept) => {
    ...
    if (responsibleChapters.length === 1) {
      ...
      if (
        ...
        responsibleChapters.includes(selectedChapterIndex)
      ) {
        layer.setStyle({
          color: color,
          weight: 2,
    ...
    onEachFeature: (feature, layer) => {
      ...
      if (deptCode) {
        ...
        CHAPTERS.forEach((ch, idx) => {
          ...
          if (
            (mode === "departments" || mode === "both") &&
            deptList.includes(deptCode)
          ) {
        ...
        if (manualCandidates.length > 0) {
          ...
          primaryDistance =
            primaryChapterIndex !== -1
              ? distancesByChapter[primaryChapterIndex]
              : Infinity;
...
async function handleZoneSearchForDept(dept, lat, lon, label) {
  ...
  for (const info of infos) {
    if (info.travelMinutes != null) {
      if (
        !best ||
        best.travelMinutes == null ||
        info.travelMinutes < best.travelMinutes
      ) {
        best = info;
      }
    }
